...
void SomethingHappened(BTH_LE_GATT_EVENT_TYPE EventType, PVOID EventOutParameter, PVOID Context)
{
	...
	if (0 == CharSize) {
		...
		f2b_t grv[3] = { 0 };
		for (uint8_t i = 0; i < 4; i++) {
			grv[0].b[i] = ValueChangedEventParameters->CharacteristicValue->Data[i];
			grv[1].b[i] = ValueChangedEventParameters->CharacteristicValue->Data[i + 4];
			grv[2].b[i] = ValueChangedEventParameters->CharacteristicValue->Data[i + 8];
		}
		printf("grv:\t%0.2f %0.2f %0.2f\n", grv[0].val, grv[1].val, grv[2].val);
	}
	else {
